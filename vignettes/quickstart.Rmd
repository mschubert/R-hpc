...
We can type `:w` <kbd>Enter</kbd> to write the file, or `:wq` to write the file
and quit the editor. `:q!` will exit without saving any changes.

This is all you need to know to make `vim` as useful as `nano`. However, there
are many more features that you can explore by running `vimtutor` from the
command-line.
