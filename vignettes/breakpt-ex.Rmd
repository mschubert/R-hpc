...
The jobs started automatically by slurmR will not inherit the same
environment used in your interactive job. This will not be a problem
if you've added module comamnds to your `.bashrc` file as in the previous
exercises. 
...
library, available as a module assuming the prerequisite modules for R have
already been loaded then;

```{sh eval=FALSE}
module load ZeroMQ/4.3.4
```
will load a compatible ZeroMQ module.

```{r eval=FALSE}
# install.packages("clustermq") if you have not installed the package yet
library(clustermq)

Q(function(i) Sys.sleep(1), i=1:10, n_jobs=1)
```
