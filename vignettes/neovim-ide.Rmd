...
To use R from within Neovim, we need to create a helper script to start R in a
job instead of on the login node (save this as `~/.local/bin/srunR` and make it
executable using `chmod +x ~/.local/bin/srunR`):

```{sh eval=FALSE}
#!/bin/sh

if [[ -z $SLURM_JOB_ID ]]; then
    srun --account su105 --pty R $@
else
    R
fi
```

Next, we can uncomment the lines previously commented in
...
Because we added an `srunR` to the `R_cmd`, we can edit files on the login node,
